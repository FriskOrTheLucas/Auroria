...
namespace Auroria.NT
{
    public partial class AurNTBase : Form
    {
        ...
        public void ShirtsListView_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ShirtsListView.SelectedItems.Count == 0)
            {
                ShrtInfoTxt.Text = "No items selected!";
                return;
            }

            string selectedShirt = ShirtsListView.SelectedItems[0].Text;
            ...
            foreach (var file in Directory.GetFiles(@"Data/Catalog/charshirts"))
            {
                if (file.EndsWith(".json"))
                {
                    ...
                    if (ShirtName == selectedShirt)
                    {
                        ShrtInfoTxt.Text = Description;
                        return;
                    }
                }
            }
