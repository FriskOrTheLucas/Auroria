...
// 4.- Figure out how to do a Json to catalog. [started]
...
namespace Auroria.NT
{
    public partial class AurNTBase : Form
    {
        ...
        private void AurNTBase_Load(object sender, EventArgs e)
        {
            ...
            // Populate the shirts list view with items from the catalog
            CatalogManager catalog = new CatalogManager();

            List<Item> shirts = catalog.GetShirts();

            ShirtsListView.Items.Clear();

            foreach (Item shirt in shirts)
            {
                ListViewItem item = new ListViewItem(shirt.Name);

                item.SubItems.Add(shirt.Catergory);
                item.SubItems.Add(shirt.Thumbnail);

                ShirtsListView.Items.Add(item);
                // please dont effing die.
            }
        }
